...
　基本的には、`JDIWarningData`が発生した場合は直後に`._update_progress`メソッドによる更新処理を行います。一方で、`JDIErrorData`が発生した場合には、`._update_progress`メソッドは呼び出さず、`.cancel`メソッドによる終了処理を行います。また、`.cancel`メソッドを呼び出す階層は、極力`._run`メソッドの直下（まわり）に限定しています。

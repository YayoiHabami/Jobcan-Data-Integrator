...
- `FormDetailApiInvalidParameter` : 申請書データ (詳細) のAPIのパラメータが不正な場合の警告情報
- `FormDetailApiDataNotFound` : 指定された申請書データ (詳細) が見つからない場合の警告情報
- `FormDetailApiUnexpectedError` : 申請書データ (詳細) のAPIで予期しないエラーが発生した場合の警告情報
...
class FormDetailApiInvalidParameter(JDIWarningData):
...
class FormDetailApiDataNotFound(JDIWarningData):
...
class FormDetailApiUnexpectedError(JDIWarningData):
...
def get_form_detail_api_warning(status_code:int, res:dict, request_id:str="") -> JDIWarningData:
    ...
    """
    if status_code == 400:
        return FormDetailApiInvalidParameter(res)
    elif status_code == 404:
        return FormDetailApiDataNotFound(request_id)
    else:
        return FormDetailApiUnexpectedError(status_code)
